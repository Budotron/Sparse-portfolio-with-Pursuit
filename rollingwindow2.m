...
warning;    
load datapoints
allData=datapoints(1:320, :);
[m,n]=size(allData)
window=0:299 ;
threshold=1e-8;
for dataSetNumber=1:20
    window=window+1;
    thisPeriodPrices=datapoints(window, :);
    nextPeriodPrices=datapoints(window+window, :);
...
%             names(selectedAssets);
        ...
        BackwardsVar(dataSetNumber, 4-sparsity_number)=...
                    calculatePortfolioVarience(backwardsPort, Sigma);     
        %%%%%%%%%%%%%%%%%% backwards portfolio returns %%%%%%%%%%%%%%%%%%%%
        realBackwardsReturns_month(dataSetNumber,4-sparsity_number)=...
        returnOnInvestment(investmentPrice, rewardPrice_1month,...
        backwardsPort);
        realBackwardsReturns_3months(dataSetNumber, 4-sparsity_number)=...
        returnOnInvestment(investmentPrice, rewardPrice_3months,...
        backwardsPort);
        realBackwardsReturns_year(dataSetNumber, 4-sparsity_number)=...
        returnOnInvestment(investmentPrice, rewardPrice_year,...
        backwardsPort);          
