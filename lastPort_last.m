...
% realBackwardsReturns_Day2=zeros(numPeriods, numPeriods);%(?)
...
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for beta=min(betarange):(max(betarange)-min(betarange))/9:max(betarange)
        ...
        %%%%%%%%%% if the backwards and forwards methods haven't been %%%%%
        %%%%%%%%%%%%%%%%%%%%%%%performed yet %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if ~isempty(ForwardsNamesMat) 
            sparsity_set=0;
            for desiredSparsity=10:10:100
                sparsity_set=sparsity_set+1;
                S=getBestIndex(muvec);
                if desiredSparsity==100
                    indexTank=[];
                else
                    indexTank=bank;
                end                
...
%                                       %numrows(allInds)=desiredSparsity
                ...
                realBackwardsReturns_Day2(targetReturnNumber, sparsity_set)=...
                returnOnInvestment(purchaseDayPrices, nextDayPrices, x);
                
            end    
        end
        realBackwardsReturns_Day2_all{dataSetNumber}=realBackwardsReturns_Day2;
